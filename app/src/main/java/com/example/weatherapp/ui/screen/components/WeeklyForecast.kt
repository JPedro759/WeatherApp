...
@Composable
fun WeeklyForecast(modifier: Modifier = Modifier, data: List<ForecastItem> = ForecastData){
...
@Composable
private fun WeatherForecastHeader(modifier: Modifier = Modifier){
    Row(
        modifier = modifier.fillMaxWidth(),
...
@Composable
private fun Forecast(modifier: Modifier = Modifier, item: ForecastItem){
    ...
    val primaryTextColor = remember(item.isSelected) {
        if(item.isSelected) Color.White else ColorTextPrimary
    }
    ...
    val temperatureTextColor = remember(item.isSelected) {
        if(item.isSelected){
            TextStyle(
                brush = Brush.linearGradient(
                    0f to Color.White,
            ...
            TextStyle(color = ColorTextPrimary, fontSize = 24.sp, fontWeight = FontWeight.Black)
        }
    }

    Column(
        modifier = updatedModifier.width(65.dp).padding(horizontal = 10.dp, vertical = 16.dp),
        ...
        Text(text = item.temperature, style = temperatureTextColor)
        Spacer(modifier = Modifier.height(8.dp))
...
@Composable
private fun WeatherImage(modifier: Modifier = Modifier, @DrawableRes image: Int){
...
@Composable
private fun AirQualityIndicator(modifier: Modifier = Modifier, value: String, color: String){
    Surface(
        modifier = modifier,
